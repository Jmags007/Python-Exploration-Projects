...
def calculate_dice_roll_for_selection(dice_roll_lst, user_selection_string):
    if user_selection_string == "ones":
        score = upper_scoring(dice_roll_lst, 1)
    if user_selection_string == "twos":
        score = upper_scoring(dice_roll_lst, 2)
    if user_selection_string == "threes":
        score = upper_scoring(dice_roll_lst, 3)
    if user_selection_string == "fours":
        score = upper_scoring(dice_roll_lst, 4)
    if user_selection_string == "fives":
        score = upper_scoring(dice_roll_lst, 5)
    if user_selection_string == "sixes":
        score = upper_scoring(dice_roll_lst, 6)
    if user_selection_string == "three of a kind":
        score = three_of_a_kind_scoring(dice_roll_lst)
    if user_selection_string == "four of a kind":
        score = four_of_a_kind_scoring(dice_roll_lst)
    if user_selection_string == "full house":
        score = full_house_scoring(dice_roll_lst)
    if user_selection_string == "small straight":
        score = small_straight_scoring(dice_roll_lst)
    if user_selection_string == "large staight":
        score = large_straight_scoring(dice_roll_lst)
    if user_selection_string == "chance":
        score = chance_scoring(dice_roll_lst)
    if user_selection_string == "yahtzee":
...
if __name__=="__main__":
    ...
    for i in range(13):
        ...
        while three_rolls_flag == True:
            ...
            if dice_roll.count(dice_roll[0]) == 5 and score_card_dict["yahtzee"] == 50:
                print("ayyyy lets go you get an additional 50 for hitting another yahtzee")
                score_card_dict["yahtzee"] = score_card_dict["yahtzee"] + 100
        ...
        flag = True
        while flag:
            user_selection = input("type the name associated with the score name you want to select for this roll: ")
            if user_selection in score_card_dict:
                if score_card_dict[user_selection] == None:
                    flag = False
            if flag == True:
        ...
        score_card_dict[user_selection] = calculate_dice_roll_for_selection(dice_roll, user_selection)
        lock.release()
